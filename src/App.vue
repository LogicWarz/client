<template>
  <v-app class="app custom-font">
    ...
    <v-dialog v-model="loading" hide-overlay persistent width="300">
      <v-card class="primary-gradient" dark>
        <v-card-text>
          Please stand by
...
export default {
  ...
  watch: {
    $route(to, from) {
      this.showNavbar = !(
        to.fullPath.includes("lobby") ||
        to.fullPath.includes("play") ||
        to.fullPath.includes("result")
      );
    }
  }
};
...
.zoom-hover {
  transition: transform 0.4s !important; /* Animation */
}
.zoom-hover:hover {
  transform: scale(
    1.1
